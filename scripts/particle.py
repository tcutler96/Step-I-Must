...
class Particle:
    def __init__(self, main, display_layer, position=(0, 0), velocity=(0, 0), velocity_min=(-1, -1), velocity_max=(1, 1), velocity_loop=(False, False), acceleration=(0, 0),
                 ...
                 glow_size=1, glow_size_min=0, glow_size_max=1, glow_size_step=0, glow_size_loop=True, glow_colour=(255, 255, 255), glow_alpha=50, glow_type=None,
        ...
        self.glow_size_step = glow_size_step
        self.glow_size_loop = glow_size_loop
        self.glow_colour = glow_colour
        self.glow_alpha = glow_alpha
        self.glow_type = glow_type
    ...
    def update(self):
        # separate into sub functions (update position, size etc.)
        ...
        if self.glow_size_step:
            self.glow_size += self.glow_size_step
            if self.glow_size >= self.glow_size_max:
                self.glow_size = self.glow_size_max
                if self.glow_size_loop:
                    self.glow_size_step *= -1
            elif self.glow_size <= self.glow_size_min:
    ...
    # def draw_glow(self, displays):  # have function that handles the drawing of both the main particle and glow effect...
    #     # pass in size, colour, alpha, position...
    #     glow_size = int(self.size + self.glow_size)
    #     glow_surface = pg.Surface((glow_size * 2, glow_size * 2))
    #     pg.draw.circle(surface=glow_surface, color=self.glow_colour, center=(glow_size, glow_size), radius=glow_size)
    #     glow_surface.set_colorkey((0, 0, 0))
    #     glow_surface.set_alpha(self.glow_alpha)
    #     displays[self.display_layer].blit(source=glow_surface, dest=(int(self.position[0] - glow_size), int(self.position[1] - glow_size)), special_flags=self.glow_type)

    def draw(self, displays):
        if int(self.size) >= 1:
            # if self.glow:
            #     self.draw_glow(displays)
