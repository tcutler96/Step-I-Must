...
const int effect_data_length = 12;
...
vec4 pixelate(sampler2D display_layer, float effect_data[effect_data_length]) {
    ...
    float pixel_height = effect_data[7] / 2 * pixel_size[1];
...
uniform float player_influence = 1.5;
...
//	float distance = (clamp(1.0 - distance(uv, (mouse_position)) * pixel_size.x / radius, 0.0, 1.0)) * player_influence * (1.25 + 0.5 * abs(sin(time / 2)));
