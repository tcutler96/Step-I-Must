        ...
        for element_type, element in self.elements.items():
            if element_type in element_types and element:
                ...
                if sprite:
                    ...
                    display_layer = 'level_player' if element_type == 'player' and element['state'] != 'dead' else 'level_dead_player' if element_type == 'player' and element['state'] == 'dead' else 'level_main'
                    displays[display_layer].blit(source=sprite, dest=(self.level_offset[0] + position[0] * self.main.sprite_size + (self.barrier_offset if element_type == 'vertical_barrier' else 0),
