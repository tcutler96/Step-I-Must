            ...
            for collectable_type, collectables in self.collectables.items():
                if collectables:
                    if self.main.debug:
                        ...
                        self.draw_cell(displays=displays, sprite=icons[collectable_type]['sprite'], offset=offset)
            if self.cryptid and ('(-1, -17)' in self.main.assets.data['game']['discovered_levels'] or self.main.debug):
                self.draw_cell(displays=displays, sprite=icons['cryptid']['sprite'], offset=offset)
